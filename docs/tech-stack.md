...
type RenderState = {
  artwork: Artwork
  transform: Transform
  baseCraft: 'none' | 'silver-glitter'
  filmCraft: 'none' | 'glossy' | 'rainbow'
  shape: 'round' | 'square'
...
- `shape` 决定圆形或方形的预览、参考线和导出裁切.
- `baseCraft` 控制闪底层: 无闪底或银闪(细闪粉粒子, 随晃动闪烁).
- `filmCraft` 控制覆膜层: 无膜、亮膜(高光反射)或素面镭射(彩虹光谱). 两层独立组合, 银闪 + 素面镭射 = 双闪.
- 三个尺寸值按当前形状分别表示直径或边长.
...
形状选项为圆形、圆角方形和心形. 所有形状复用同一套尺寸预设、图片变换和导出分辨率计算.
...
- 无闪底 + 无膜 = 纯图案, 只保留边缘弯曲暗影和投影.
- 银闪底使用 `screen` 混合的 280 个确定性伪随机闪粉粒子, 粒子亮度随 tilt 值变化.
- 亮膜使用 `screen` 混合的移动高光和 `soft-light` 全面提亮.
- 素面镭射使用 `screen` 混合的 36 扇形 conic 彩虹、`screen` 移动高光和 `overlay` 边缘虹彩环三层叠加. 默认静止(tilt=0), 打开"动态预览"后吧唧做圆形摆动, 镭射光影随摆动角度变化.
- 闪底和覆膜两层独立叠加, 银闪 + 素面镭射 = 双闪.
